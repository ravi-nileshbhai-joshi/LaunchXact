...
const FROM_AUDIT_EMAIL = process.env.RESEND_FROM_EMAIL || 'LaunchXact Deep Audit <onboarding@resend.dev>';
...
export async function POST(request) {
    try {
        ...
        try {
            ...
            console.warn('Primary model failed, trying fallback openai/gpt-oss-20b:', modelErr.message);
            chatCompletion = await groq.chat.completions.create({
                messages: [
                    { role: 'system', content: AUDIT_SYSTEM_PROMPT },
                    {
